...
export default function QuoteForm({ leadId, lead, companyId, onOpenPreview, editQuote, onEditConsumed }) {
    ...
    const handleQrFileChange = (e) => {
        ...
        if (file) {
            ...
            reader.onload = (ev) => {
                setForm((p) => ({ ...p, paymentQrUrl: ev.target.result }));
            };
        ...
        <div className="space-y-4">
            ...
            {activeTab === "form" ? (
                <div className="space-y-4">
                    ...
                    <Section title="4. Tech Stack" defaultOpen={false}>
                        <div className="flex flex-col gap-2">
                            {form.techStack.map((tech, idx) => (
                                <div key={idx} className="flex items-center gap-1 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 w-full">
                                    <input value={tech.label} onChange={(e) => setForm((p) => ({ ...p, techStack: p.techStack.map((t, i) => i === idx ? { ...t, label: e.target.value } : t) }))}
                                        className="bg-transparent text-sm text-blue-800 outline-none w-full flex-1 placeholder:text-blue-300" placeholder="e.g. React" />
                                    {form.techStack.length > 1 && (
                                        <button type="button" onClick={() => setForm((p) => ({ ...p, techStack: p.techStack.filter((_, i) => i !== idx) }))}
                                            className="text-blue-400 hover:text-red-500 transition-colors ml-2"><X size={14} /></button>
                                    )}
                                </div>
                            ))}
                            <button type="button" onClick={() => setForm((p) => ({ ...p, techStack: [...p.techStack, { label: "" }] }))} className={`${btnGhost} self-start`}>
                    ...
                    {/* Section 7: Payment Terms & QR */}
                    <Section title="7. Payment Terms & QR (Optional)" defaultOpen={false}>
                        <div className="space-y-4">
                            <div>
                                <p className="text-[11px] text-gray-400 -mt-1 mb-2">
                                    Override default payment terms. Enter each term on a new line. Leave blank to use defaults.
                                </p>
                                <textarea
                                    value={form.paymentTerms || ""}
                                    onChange={(e) => setForm((p) => ({ ...p, paymentTerms: e.target.value }))}
                                    placeholder="e.g. 1st installment 40% in advance..."
                                    rows={4}
                                    className={`${inp} resize-none`}
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-gray-700 mb-1">
                                    Payment QR (Image URL, UPI ID, or Upload)
                                </label>
                                <p className="text-[11px] text-gray-400 mb-2">
                                    Leave blank to use default. Enter a UPI ID, paste an image URL, or upload a QR image.
                                </p>
                                <div className="flex items-center gap-2">
                                    <input
                                        type="text"
                                        value={form.paymentQrUrl || ""}
                                        onChange={(e) => setForm((p) => ({ ...p, paymentQrUrl: e.target.value }))}
                                        placeholder="e.g. vivek@ybl or https://.../qr.png"
                                        className={inp}
                                    />
                                    <label className="shrink-0 px-3 py-2 text-xs font-medium bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 cursor-pointer border border-gray-300">
                                        Upload Image
