...
export default function QuoteProfileManager({ isOpen, onClose, onSave }) {
    ...
    const startEdit = (p) => {
        ...
        setForm({
            ...
            policies: p.policies ? p.policies.join("\n") : "",
    ...
    const handleSave = async () => {
        ...
        try {
            setSaving(true);
            const payload = { ...form };
            payload.policies = form.policies ? form.policies.split("\n").map(x => x.trim()).filter(Boolean) : [];
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-7xl max-h-[92vh] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {deletedCount > 0 && (
                                    <button
                                        ...
                                        className={`text-[10px] px-2 py-1 rounded-md font-medium transition ${showDeleted
                        ...
                        <div className="flex-1 overflow-y-auto p-2 space-y-1">
                            {loading ? (
                                ...
                                filteredProfiles.map((p) => (
                                    <div
                                        key={p._id}
                                        className={`p-3 rounded-lg border text-sm ${editing === p._id
                        ...
                        {!editing ? (
                            ...
                            <div className="space-y-4 max-w-xl">
                                ...
                                <div className="space-y-3 p-3 border border-amber-100 bg-amber-50/30 rounded-xl">
                                    ...
                                    <div>
                                        <label className="text-xs text-gray-500">Our Policies (One per line)</label>
                                        <textarea className={`${inp} resize-none text-xs`} rows={4} value={form.policies} onChange={(e) => set("policies", e.target.value)} placeholder="Enter policies, each on a new line. Use {companyName} for dynamic company name." />
                                    </div>
                                </div>
